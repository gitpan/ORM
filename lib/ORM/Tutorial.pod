...
=item * Condition C<( !$self-E<gt>id )> means than object is not yet stored in
database table and therefore doesn't have ID assigned to it.
In another words this means than condition is true only in C<new> method.
...
When you need to refresh object's contents simply call C<$task-E<gt>refresh>.
